...
describe("useCampaignDetail", () => {
  ...
  const mockProducts = [
    createMockFullProduct({ id: "product-1", campaignId: "campaign-1" }),
  ];
  ...
  describe("Campaign Update Handlers", () => {
    it("should handle name save", async () => {
      vi.mocked(campaignApi.update).mockResolvedValue({
        ...mockCampaign,
        name: "Updated Name",
      });
      vi.mocked(campaignApi.getBySlug).mockResolvedValue({
  ...
  describe("Keyboard Handlers", () => {
    ...
    it("should cancel description editing on Escape key", () => {
      ...
      act(() => {
        result.current.handleDescriptionKeyDown({
          key: "Escape",
        } as React.KeyboardEvent);
      });
  ...
  describe("Status Handlers", () => {
    it("should handle reopen campaign with orders", async () => {
      ...
      vi.mocked(orderApi.getByCampaign).mockResolvedValue(orders);
      vi.mocked(campaignApi.updateStatus).mockResolvedValue({
        ...mockCampaign,
        status: "CLOSED",
      });
      ...
      await waitFor(() => {
        expect(campaignApi.updateStatus).toHaveBeenCalledWith(
          "campaign-1",
          "CLOSED"
        );
      });
  ...
  describe("Edge Cases", () => {
    it("should handle missing campaignId", () => {
      ...
      const { result } = renderHook(() => useCampaignDetail(), {
